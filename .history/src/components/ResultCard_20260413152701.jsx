...
const ResultCard = forwardRef(({ code, title, description, subDimensions, userName, role }, ref) => {
  ...
  return (
    ...
    >
      ...
      {/* 头部信息 */}
      <div className="flex justify-between items-start border-b-2 border-[var(--color-primary)] pb-3 mb-4">
      ...
      {/* 代表人物区域 */}
      {role && (
        <div className="bg-[var(--color-bg-card)] border border-[var(--color-primary)] p-3 mt-4 mb-4 flex items-center gap-4 relative overflow-hidden">
          <div className="absolute top-0 right-0 w-8 h-8 bg-[var(--color-primary)] opacity-20 pointer-events-none" style={{ clipPath: 'polygon(100% 0, 0 0, 100% 100%)' }}></div>
          
          <div className="w-16 h-16 sm:w-20 sm:h-20 border-2 border-[var(--color-primary)] shrink-0 overflow-hidden relative group bg-black p-0.5">
            <div className="absolute inset-0 bg-[var(--color-primary)] opacity-20 group-hover:opacity-0 transition-opacity z-10 pointer-events-none mix-blend-color"></div>
            <img 
              src="/avatar/Linus.png" 
              alt={role} 
              className="w-full h-full object-cover filter grayscale contrast-125 brightness-90"
            />
          </div>
          <div className="flex-1 min-w-0">
            <div className="text-[10px] sm:text-xs text-[var(--color-accent-cyan)] font-bold mb-1">
              {'>'} 代表人物 (REPRESENTATIVE)
            </div>
            <div className="text-sm sm:text-base font-bold truncate" style={{ textShadow: '0 0 4px #ff9900' }}>
              {role}
            </div>
          </div>
        </div>
